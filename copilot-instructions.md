...
- **Hash caching**: Automatic caching in `HashCache.json` with 1000-entry limit and [CACHED] markers
...
2. **Batch flow**: File list → Pre-flight checks → Parallel job spawning (1-8 threads) → Real-time result writing → Progress polling (250ms timer) → Temp file streaming → Display/export with [CACHED] markers
3. **Verification flow**: Hash list input (manual or imported) → Parse (tab/space delimited) → Cache lookup → Compute hashes → Real-time result streaming → Compare → Report (MATCH/MISMATCH/MISSING)
4. **Drag-drop flow**: DragEnter → Green highlight → DragDrop → Restore color → Network path check → Populate input
5. **Export-Import flow**: Batch results → Export Verify Log → Import Batch Log button → Verify tab auto-population
...
- Verifies connectivity with configurable timeout (default 5s, range 1-30s)
- Uses background jobs with timeout to prevent hanging on unresponsive servers
- Visual feedback in footer (green=connected, red=inaccessible/timeout, orange=checking)
- Pre-flight checks in batch operations detect and handle inaccessible network files
- Timeout value configurable in Settings tab and persisted to config file
...
Saves: algorithm selection, dark mode state, font size, hash format, auto-copy preference, recent files list (max 50), parallel thread count, network path timeout, system tray preferences, toast notification settings.
...
11. **Real-time streaming**: Batch and verify operations write results immediately to temp files for live UI updates
12. **Horizontal scrolling**: ListBox HorizontalExtent must be recalculated when items change or font size changes
...
- **Batch**: Mix of small/large files, test parallel threads (1-8), verify progress updates, test stop button, verify real-time result streaming as files complete
- **Verify**: Test tab/space delimited formats, relative/absolute paths, missing files, CRC32 8-char hashes, verify progress bar updates, test Import Batch Log button, verify real-time streaming
- **Dark mode**: Toggle and verify all 8 tabs + nested controls render correctly, check drag-drop color restoration
- **Recent files**: Add >50 files to test limit, test dual-button enable/disable logic with 0/1/multiple selections, remove files from disk to test validation, verify cache provides instant results, test horizontal scrolling at different font sizes
...
- **Cache behavior**: Hash same file twice, verify instant second result and [CACHED] marker; modify file, verify cache invalidates; test 1000-entry limit
- **Parallel scaling**: Batch hash 20+ files, test thread counts 1/4/8, measure completion time differences, verify real-time result appearance
- **Export formats**: Test TXT/CSV export, HashCheck format (.sha256 files), SFV format for CRC32, Verify Log export with [CACHED] marker removal
- **Import-Export cycle**: Export batch results as Verify Log → Import in Verify tab → Verify hashes → Check for [CACHED] marker issues
- **Horizontal scrolling**: Add files with long paths to batch list, adjust font size 8-24pt, verify scrolling works at all sizes
...
- Hash caching system with automatic invalidation and [CACHED] markers
...
- Configurable parallel batch processing (1-8 threads) with real-time result streaming
- PlanetArchives dark mode with bold rich colors
- System tray minimization with toast notifications
- Export to multiple formats (TXT/CSV, HashCheck, SFV, Verify Log)
- Enhanced Recent Files with smart dual-button system and horizontal scrolling
