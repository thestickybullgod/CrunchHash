...
function Export-SFVFile {
    ...
    try {
        $sfvContent = "; Generated by Hashps1 v2.3 on $([DateTime]::Now.ToString('yyyy-MM-dd HH:mm:ss'))`r`n"
...
"@
        ...
        [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier("Hashps1 v2.3").Show($toast)
    } catch {
...
function Initialize-TrayIcon {
    ...
    try {
        ...
        $script:notifyIcon.Text = "Hashps1 v2.3"
...
if ($script:PortableMode) {
    $form.Text = "Hashps1 v2.3  [WARNING: Portable Mode - No config/cache will be saved]"
} else {
    $form.Text = "Hashps1 v2.3"
}
...
$buttonExportVerifyLog.Location = New-Object System.Drawing.Point(430, 543)
...
$checkBatchLog.Location = New-Object System.Drawing.Point(20, 580)
$checkBatchLog.AutoSize = $true

$labelBatchFooter = New-Object System.Windows.Forms.Label
$labelBatchFooter.Text = "Ready"
$labelBatchFooter.Location = New-Object System.Drawing.Point(120, 580)
...
$panelBatchProgressBackground.Location = New-Object System.Drawing.Point(20, 608)
...
$buttonImportBatchLog.Add_Click({
    ...
    if ($dialog.ShowDialog() -eq "OK") {
        try {
            $logContent = Get-Content -Path $dialog.FileName -Raw
            
            # Parse batch log format: "filename: hash"
            # Extract hash-filename pairs
            $lines = $logContent -split "`r?`n" | Where-Object { $_ -match '^\s*(.*?):\s+([a-fA-F0-9]+)\s*$' }
            
            if ($lines.Count -eq 0) {
                [System.Windows.Forms.MessageBox]::Show("No valid hash entries found in the log file.`n`nExpected format: filename: hash", "Import Failed", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
                return
            }
            
            # Convert to hash<tab>filename format
            $importedLines = @()
            foreach ($line in $lines) {
                if ($line -match '^\s*(.*?):\s+([a-fA-F0-9]+)\s*$') {
                    $filename = $matches[1].Trim()
                    $hash = $matches[2].Trim()
                    $importedLines += "$hash`t$filename"
                }
            }
            
            # Set the text box with imported data
            $textBoxVerifyInput.Text = $importedLines -join "`r`n"
            
            [System.Windows.Forms.MessageBox]::Show("Successfully imported $($importedLines.Count) hash entries from batch log.`n`nClick ""Verify All"" to verify hashes.", "Import Complete", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
        } catch {
...
$buttonVerify.Add_Click({
    ...
    foreach ($line in $lines) {
            ...
            if (-not (Test-Path $fullPath)) {
                ...
                try {
                    ...
                    if ($computedHash -eq $normalizedExpected) {
                        ...
                        $resultLine = "MISMATCH: $filename (expected: $expectedHash, got: $computedHash)"
                        $mismatchCount++
                    }
                } catch {
        ...
        if ($textBoxVerifyResults.Text -eq "Verifying files...`r`n") {
            $textBoxVerifyResults.Text = $resultLine + "`r`n"
        } else {
            $textBoxVerifyResults.AppendText($resultLine + "`r`n")
        }
        [System.Windows.Forms.Application]::DoEvents()
    }
...
$buttonExportVerifyLog.Add_Click({
    ...
    if ($saveDialog.ShowDialog() -eq "OK") {
        try {
            ...
            foreach ($line in $lines) {
                # Match format: hash<tab>Size:...<tab>Modified:...<tab>filepath<optional:[CACHED]>
                if ($line -match '^([a-fA-F0-9]+)\tSize: .+?\tModified: .+?\t(.+?)(?:\t\[CACHED\])?$') {
                    $hash = $matches[1]
                    $filePath = $matches[2].Trim()
                    # Remove [CACHED] marker if it's still there
...
$form.Add_Resize({
    if ($form.WindowState -eq 'Minimized' -and $checkMinimizeToTray.Checked) {
        ...
        if ($script:currentJobId -or $script:batchJobId) {
            $script:notifyIcon.BalloonTipTitle = "Hashps1 v2.3"
