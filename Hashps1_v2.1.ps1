...
$buttonImportBatchLog.Add_Click({
    ...
    if ($dialog.ShowDialog() -eq "OK") {
        try {
            ...
            [System.Windows.Forms.MessageBox]::Show("Successfully imported $($importedLines.Count) hash entries from batch log.", "Import Complete", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
        } catch {
    ...
    if ($saveDialog.ShowDialog() -eq "OK") {
        try {
            ...
            foreach ($line in $lines) {
                ...
                if ($line -match '^([a-fA-F0-9]+)\t.+\t(.+?)(?:\t\[CACHED\])?$') {
                    $hash = $matches[1]
