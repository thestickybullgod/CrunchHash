...
$script:batchFileProgressFile = [System.IO.Path]::Combine($PSScriptRoot, "batch_file_progress.tmp")
$script:batchCacheFile = [System.IO.Path]::Combine($PSScriptRoot, "batch_cache.tmp")
$script:batchTotalFiles = 0
$script:batchCurrentFile = 0
$script:batchFileProgress = 0
$script:verifyJobId = $null
$script:verifyTempFile = [System.IO.Path]::Combine($PSScriptRoot, "verify_results.tmp")
$script:verifyProgressFile = [System.IO.Path]::Combine($PSScriptRoot, "verify_progress.tmp")
...
function Export-SFVFile {
    ...
    try {
        $sfvContent = "; Generated by Hashps1 v2.5 on $([DateTime]::Now.ToString('yyyy-MM-dd HH:mm:ss'))`r`n"
...
"@
        ...
        [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier("Hashps1 v2.5").Show($toast)
    } catch {
...
function Initialize-TrayIcon {
    ...
    try {
        ...
        $script:notifyIcon.Text = "Hashps1 v2.5"
...
if ($script:PortableMode) {
    $form.Text = "Hashps1 v2.5  [WARNING: Portable Mode - No config/cache will be saved]"
} else {
    $form.Text = "Hashps1 v2.5"
}
...
$buttonBatchHash.Add_Click({
    ...
    if ($inaccessibleNetworkFiles.Count -gt 0) {
        ...
        foreach ($file in $inaccessibleNetworkFiles) {
            $listBoxBatchFiles.Items.Remove($file)
        }
...
$form.Add_Resize({
    if ($form.WindowState -eq 'Minimized' -and $checkMinimizeToTray.Checked) {
        ...
        if ($script:currentJobId -or $script:batchJobId) {
            $script:notifyIcon.BalloonTipTitle = "Hashps1 v2.5"
