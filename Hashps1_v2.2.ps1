...
function Export-SFVFile {
    ...
    try {
        $sfvContent = "; Generated by Hashps1 v2.2 on $([DateTime]::Now.ToString('yyyy-MM-dd HH:mm:ss'))`r`n"
...
"@
        ...
        [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier("Hashps1 v2.2").Show($toast)
    } catch {
...
function Initialize-TrayIcon {
    ...
    try {
        ...
        $script:notifyIcon.Text = "Hashps1 v2.2"
...
$buttonGenerate.Add_Click({
    ...
    if ($radioFile.Checked) {
        ...
        try {
            ...
            # Format as single line for horizontal scrolling
...
$form.Add_Resize({
    if ($form.WindowState -eq 'Minimized' -and $checkMinimizeToTray.Checked) {
        ...
        if ($script:currentJobId -or $script:batchJobId) {
            $script:notifyIcon.BalloonTipTitle = "Hashps1 v2.2"
