...
"@
            ...
            foreach ($file in $files) {
                ...
                try {
                    if (Test-Path $file -ErrorAction SilentlyContinue) {
                        ...
                        try {
                            ...
                            if ($fileItem) {
                                ...
                                $fileSizeStr = if ($fileSize -lt 1KB) { "$fileSize bytes" } 
                                               elseif ($fileSize -lt 1MB) { "{0:N2} KB" -f ($fileSize / 1KB) }
                                               elseif ($fileSize -lt 1GB) { "{0:N2} MB" -f ($fileSize / 1MB) }
                                               else { "{0:N2} GB" -f ($fileSize / 1GB) }
                                $fileModified = $fileItem.LastWriteTime.ToString("yyyy-MM-dd HH:mm:ss")
                            }
                            
                            $resultLine = "$hash`tSize: $fileSizeStr`tModified: $fileModified`t$file"
                        ...
                        $resultLine = "ERROR: File not found`tSize: N/A`tModified: N/A`t$file"
                        try {
                            [System.IO.File]::AppendAllText($tempFile, $resultLine + [Environment]::NewLine)
                        } catch { }
                    }
                    ...
                    $resultLine = "ERROR: $errMsg`tSize: $fileSizeStr`tModified: $fileModified`t$file"
                    try {
                        [System.IO.File]::AppendAllText($tempFile, $resultLine + [Environment]::NewLine)
                    } catch { }
                }
...
$buttonGenerate.Add_Click({
    ...
    if ($radioFile.Checked) {
        ...
        try {
            ...
            if ($checkLog.Checked) {
                $timestamp = [DateTime]::Now.ToString("yyyy-MM-dd HH:mm:ss")
                $line = "$timestamp | Mode: String | Input: $input | Algo: $algoName | Format: $selectedFormat | Size: N/A | Modified: N/A | Hash: $hash"
                [System.IO.File]::AppendAllText($logPath, $line + [Environment]::NewLine)
            }
        } catch {
