...
function Export-SFVFile {
    ...
    try {
        $sfvContent = "; Generated by CrunchHash v2.8 on $([DateTime]::Now.ToString('yyyy-MM-dd HH:mm:ss'))`r`n"
...
"@
        ...
        [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier("CrunchHash v2.8").Show($toast)
    } catch {
...
function Initialize-TrayIcon {
    ...
    try {
        ...
        $script:notifyIcon.Text = "CrunchHash v2.8"
...
if ($script:PortableMode) {
    $form.Text = "CrunchHash v2.8  [WARNING: Portable Mode - No config/cache will be saved]"
} else {
    $form.Text = "CrunchHash v2.8"
}
...
$labelHeader.Text = "CrunchHash v2.8"
...
# Block user interaction during scan - keep cursor as arrow and clear selections
$textBoxDupResults.Add_MouseMove({
    param($sender, $e)
    if ($script:dupScanRunning) {
    ...
    try {
        ...
        if (-not [string]::IsNullOrWhiteSpace($resultLine)) {
            ...
            try {
                ...
                # If UI update fails, silently continue processing
            }
        }
    }
    } catch {
...
$buttonDupFind.Add_Click({
    ...
    [void]$script:dupRunspace.AddScript({
        param($paths, $isRecursive, $algo, $tempFile, $progressFile, $extensionFilter, $stopFile, $hashCachePath)

        try {
            # Write initial progress immediately to signal start
            "0|0|STARTING" | Out-File -FilePath $progressFile -Force -Encoding UTF8
            ...
            foreach ($path in $paths) {
                "0|0|SCANNING: $path" | Out-File -FilePath $progressFile -Force -Encoding UTF8
                ...
                foreach ($file in $fileEnum) {
                    ...
                    if ($totalEnumerated - $lastUpdateCount -ge 100) {
                        "0|$totalEnumerated|ENUMERATING" | Out-File -FilePath $progressFile -Force -Encoding UTF8
                        $lastUpdateCount = $totalEnumerated
                    }
                }
            }

            "0|$totalEnumerated|GROUPING" | Out-File -FilePath $progressFile -Force -Encoding UTF8
            ...
            if (Test-Path $script:dupProgressFile) {
                try {
                    ...
                    if ($parts.Count -ge 3) {
                        ...
                        # Show status in footer and update appropriate progress bar
                        if ($status -like "SCANNING:*") {
                            $labelDupFooter.Text = $status
                            $labelDupProgressPercent.Text = "10%"
                            $panelDupEnumFill.Width = [int]($panelDupProgressBackground.Width * 0.1)  # 10% for scanning
                        } elseif ($status -eq "ENUMERATING") {
                            $labelDupFooter.Text = "Enumerating files... ($total found so far)"
                            $labelDupProgressPercent.Text = "50%"
                            # Pulse the blue bar during enumeration (33-66% range)
                            $pulseWidth = [int]($panelDupProgressBackground.Width * 0.5)
                            $panelDupEnumFill.Width = $pulseWidth
                        } elseif ($status -eq "FILTERING") {
                            $labelDupFooter.Text = "Filtering files... ($total found)"
                            $labelDupProgressPercent.Text = "75%"
                            $panelDupEnumFill.Width = [int]($panelDupProgressBackground.Width * 0.75)  # 75% for filtering
                        } elseif ($status -eq "GROUPING") {
                            $labelDupFooter.Text = "Grouping by size... ($total files)"
                            $labelDupProgressPercent.Text = "0%"  # Reset to 0% after enumeration completes
                            $panelDupEnumFill.Width = $panelDupProgressBackground.Width  # 100% - enum complete
                            ...
                            # Hashing phase - hide blue bar and show green bar progress
                            $panelDupEnumFill.Width = 0  # Hide blue bar during hashing phase
...
$form.Add_Resize({
    if ($form.WindowState -eq 'Minimized' -and $checkMinimizeToTray.Checked) {
        ...
        if ($script:currentJobId -or $script:batchJobId) {
            $script:notifyIcon.BalloonTipTitle = "CrunchHash v2.8"
...
# Explicitly exit to prevent any output
exit